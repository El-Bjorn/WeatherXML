...
if ($#ARGV <5){
	print STDERR "Too few args. Required args: latitude longitude numHours city state directoryPath\n";
	exit(0);
}
...
print STDERR "resource path: $resource_path\n";
...
open(XMLFILE,">$resource_path/forecast.xml") || die "Can't open forecast.xml in $resource_path: $!";
print XMLFILE $xml;
close (XMLFILE);
...
$pred_index=0;
$additive_sect = 0; # number of additive sections in the forecast
foreach $node (@w_node_list){
	$_ = XML::XPath::XMLParser::as_string($node);
	#print STDERR "parsing WC line: $_\n";
	if (/additive/){ # additive section of forecast
		$pred_index--; # roll back the index
		$additive_sect++;
		$addIndex = $additive_sect+1;
		#print STDERR "processing additive section $additive_sect\n";
		/coverage="(.*)" intensity="(.*)" additive="(.*)" weather-type="(.*)" qualifier="(.*)"/;
		$cov_key = "coverage".$addIndex;
		$weather_pred[$pred_index]{$cov_key} = $1;  
		$intense_key = "intensity".$addIndex;
		$weather_pred[$pred_index]{$intense_key} = $2;  
		$type_key = "type".$addIndex;
		$weather_pred[$pred_index]{$type_key} = $4;  
		$qual_key = "qualifier".$addIndex;
		$weather_pred[$pred_index]{$qual_key} = $5;  
		$weather_pred[$pred_index]{numAdditives} = $additive_sect;
		$pred_index++; # roll it forward again
	} else { # first part, don't know if we have 1 or 2 parts
		$additive_sect = 0; # reset additive
		/coverage="(.*)" intensity="(.*)" weather-type="(.*)" qualifier="(.*)"/;
		$weather_pred[$pred_index] = { type => $3,
						qualifier => $4,
						intensity => $2,
						coverage => $1 };
		$weather_pred[$pred_index]{numAdditives} = 0;
...
if ($num_preds==0){
	print STDERR "noaa has failed us, 0 weather predictions... oh well...continuing\n";
	$NOAA_sucks = 1;
}
if (!$NOAA_sucks ){
...
## Generate the plist ---------------------------------------------
open(PLIST,">$resource_path/$plist_filename") || die "can't open $plist_filename in $resource_path: $!";
# header BS
